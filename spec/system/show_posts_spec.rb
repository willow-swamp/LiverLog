  ...
  describe "ポスト一覧の表示" do
    ...
    context "飲酒日を記録する時" do
      it "ポストが作成される" do
        ...
        click_button "登録"
        visit group_path(group)
        expect(page).to have_content "#{user.username}さんがお酒を嗜みました🍺（今日のアルコール摂取量：#{ApplicationController.helpers.alcohol_caluculate(user.posts.last.drink_record.drink_volume, user.posts.last.drink_record.alcohol_percentage)}g）"
        expect(page).to have_content user.posts.last.drink_record.start_time.strftime("%F")
      end
    end
  end
  ...
  describe "ポスト詳細の表示" do
    ...
    context "休肝日の記録がされている時" do
      it "ポスト詳細が表示される" do
        ...
        expect(page).to have_content post.drink_record.start_time.strftime("%F")
